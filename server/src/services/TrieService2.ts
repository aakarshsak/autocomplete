import Trie2 from "../db/trie2";

export const insert = async (word: string) => {
  let root = await Trie2.findOne({ isRoot: true });
  if (!root) {
    root = new Trie2({ prefix: "", isRoot: true });
    await root.save();
  }
  ...
  for (let char of word) {
    ...
    for (let child of currentNode?.children) {
      childNode = await Trie2.findOne({ _id: child, prefix: char });
      if (childNode) break;
    }

    if (!childNode) {
      const newNode = new Trie2({ prefix: char });
      await newNode.save();
...
export const allCombination = async (
  ...
  for (let child of startingNode.children) {
    const childNode = await Trie2.findById(child);
